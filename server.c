...
void process_client_req(struct io_uring *ring, int fd, size_t size);
...
int main(int argc, char *argv[]) {
    ...
    printf("Start listening (pass 'q' to stop)...\n");
        ...
        for (int i = 0; i < cqe_count; ++i) {
            ...
            if (type == ACCEPT) {
                ...
                if (bytes_read <= 0) {
                    ...
                    if (strlen(bufs[user_data->fd]) == 2 && bufs[user_data->fd][0] == 'q') {
                        shutdown(user_data->fd, SHUT_RDWR);
                    ...
                    process_client_req(&ring, user_data->fd, bytes_read);
                }
